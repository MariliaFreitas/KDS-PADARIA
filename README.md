...
- [x] **Etapa 15 — Histórico** (histórico operacional de pedidos concluídos ou cancelados; `GET /api/history/orders` com paginação, busca por número operacional ou nome do cliente e filtros por período/status; detalhe completo em `GET /api/history/orders/:orderId` com itens, valores, datas e timeline baseada em `OrderHistory`; eventos de criação do pedido, inclusão e mudança de status de itens, pagamento e entrega; acesso restrito a CAIXA/ADMIN; telas `/history` e `/history/orders/:orderId`)
- [x] **Etapa 16 — Tempo real** (atualização automática das telas operacionais via Server-Sent Events — sem WebSocket, sem polling periódico da API; `GET /api/realtime/events`, autenticado — qualquer perfil pode abrir a conexão, mas cada evento chega reduzido aos escopos que aquele perfil já teria acesso via REST (mesmo mapeamento de `authorize()`: ATENDENTE→orders, PRODUCAO→production, CAIXA→cashier+delivery, ADMIN→orders+production+cashier — delivery continua restrito a CAIXA, ADMIN não o recebe aqui também); o stream só carrega sinais mínimos de invalidação (`scopes`, `orderId`/`stationId` opcionais, `timestamp`) e nunca dado de pedido — preço, nome de cliente, adicionais e pagamento nunca aparecem no evento; REST continua sendo a única fonte da verdade: ao receber um evento relevante, a tela refaz o mesmo GET que já fazia; publicação acontece sempre depois da transação que altera o banco ter commitado — nunca antes, nunca numa operação que falhou; um assinante que falha ao receber (ex: escrita num socket já quebrado) nunca afeta o publisher nem os demais assinantes — só a própria conexão quebrada é encerrada; heartbeat periódico mantém a conexão viva atrás de proxy; frontend consome via `fetch` + `ReadableStream`, com o token no header `Authorization` (nunca `EventSource` nativo, que exigiria o token na URL), com reconexão automática por backoff curto e limitado, refazendo o GET completo sempre que a conexão volta de uma instabilidade — mesmo quando essa instabilidade aconteceu antes da primeira conexão bem-sucedida; telas conectadas: `/production`, `/production/stations/:stationId`, `/cashier`, `/delivery`, `/orders/:orderId`)
